...
const FormEditRole = () => {
  const { id } = useParams<{ id: string }>(); // ambil id role dari route param
  ...
  useEffect(() => {
    ...
    axios.get(`/api/roles/${id}`)
      ...
      .catch(() => {
        ...
        navigate("/form-role"); // kembali ke list role jika gagal
      })
      .finally(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const response = await axios.put(`/api/roles/update/${id}`, {
        name: name.trim(),
      ...
      alert(msg || "Terjadi kesalahan saat memperbarui data.");
    } finally {
      setLoading(false);
    }
  };

  if (loadingRole) return <p>Memuat data role...</p>;

  return (
    <>
      <Breadcrumb pageName="Form Edit Role" />
      <div className="flex flex-col gap-6">
        <div className="flex justify-between items-center px-6 py-4 border-b border-stroke dark:border-strokedark">
          <h3 className="text-lg font-semibold text-black dark:text-white">Form Edit Role</h3>
          <Link
            to="/form-role"
