...
export default function FormInputUser() {
  ...
  const [role, setRole] = useState(''); // <-- ini akan menyimpan role.name
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        email,
        password,
        role, // kirim role sebagai string, contoh "admin"
      };
      ...
      console.log(error.response?.data); // tampilkan error Laravel 422
      alert(message);
    }
  };
  ...
  return (
    <>
      <Breadcrumb pageName="Input User" />
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div>
            ...
            <input
              ...
              required
              className="w-full rounded border border-stroke bg-white py-2 px-4 text-black 
          ...
          <div className="relative">
            <label className="mb-2 block text-black dark:text-white">Password</label>
            <input
              type={showPassword ? 'text' : 'password'}
              value={password}
              onChange={e => setPassword(e.target.value)}
              required
              className="w-full rounded border border-stroke bg-white py-2 px-4 pr-10 text-black 
                         dark:border-form-strokedark dark:bg-form-input dark:text-white"
            />
            <button
              type="button"
